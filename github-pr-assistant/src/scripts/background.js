...
// IMPORTANT: Set your API keys in the extension options page
const CONFIG = {
  github: {
...
chrome.storage.local.get(['apiConfig'], (result) => {
  if (result.apiConfig) {
    ...
    Object.assign(CONFIG.openai, result.apiConfig.openai || {});
  }
});
...
async function generateAISummary(prDetails) {
  ...
  if (!CONFIG.features.realAIAnalysis || CONFIG.openai.apiKey === 'YOUR_OPENAI_API_KEY') {
    console.log('⚠️ OpenAI API not configured. Using mock analysis.');
    return generateMockAnalysis(pr, files, metadata);
  }

  try {
    console.log('🤖 Generating AI summary...');
    const aiResponse = await callOpenAI(pr, files);
...
async function callOpenAI(pr, files) {
  ...
  const fileSummaries = files.slice(0, 20).map(file => ({
    ...
    // Limit patch to first 500 characters to stay within token limits
    patch: file.patch ? file.patch.substring(0, 500) + (file.patch.length > 500 ? '...' : '') : 'No patch available'
  }));

  // Build the prompt
  const systemPrompt = `You are an expert code reviewer and software architect.
Your task is to analyze pull requests and provide actionable insights for reviewers.

IMPORTANT RULES:
1. ONLY reference files that are explicitly listed in the "Files Modified" section - never invent or assume file paths
2. ONLY mention security concerns if the changes actually involve security-sensitive code (auth, encryption, validation, permissions)
3. Be accurate about impact levels: small visual/doc changes = LOW, routine features = MEDIUM, breaking changes/major features = HIGH
4. Focus on the actual changes shown, not hypothetical scenarios

Respond in JSON format with these fields:
{
  "summary": "2-3 sentence overview of what this PR does and why",
  "keyChanges": ["list", "of", "3-5", "key changes"],
  "impactDescription": "Description of the impact this PR will have",
  "checklist": ["list", "of", "4-6", "review items"],
  "potentialIssues": ["list of potential concerns if any"],
  "securityConcerns": ["ONLY include if actual security-sensitive code is modified - otherwise return empty array"]
}`;
...
- ${f.filename} (${f.status})
  ...
  - Changes preview: ${f.patch}
`).join('\n')}

Provide a comprehensive analysis in JSON format.`;

  try {
    const response = await fetch(`${CONFIG.openai.apiBase}/chat/completions`, {
      method: 'POST',
      headers: {
